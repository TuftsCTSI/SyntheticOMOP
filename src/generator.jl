...
function parse_date_opt(s)::Union{Date, Missing}
    return s === nothing ? missing : parse_date(s)
end
...
function resolve_opt(concepts::Dict, node::Dict, key::String)::Union{Int, Missing}
    ...
    return value === nothing ? missing : resolve(concepts, value)
end

function resolve_type(concepts::Dict, node::Dict)::Int
    value = get(node, "type_concept_id", nothing)
    return value === nothing ? TYPE_EHR : resolve(concepts, value)
end
...
function _group_events_into_visits(patient::Dict, concepts::Dict)::Vector{VisitGroup}
    ...
    return sort(collect(values(groups)); by = g -> g.date)
end
...
function process_patient!(state::BuildState, patient::Dict, pid::Int)
    ...
    push!(
        state.accum[:observation_period],
    ...
    for g in visit_groups
        ...
        for spec in g.drugs
            push!(state.accum[:drug_exposure], build_drug(spec, pid, vid, state.concepts, state.counters[:drug_exposure]))
        end
        for spec in g.procedures
            push!(state.accum[:procedure_occurrence], build_procedure(spec, pid, vid, state.concepts, state.counters[:procedure_occurrence]))
        end
        for spec in g.devices
            push!(state.accum[:device_exposure], build_device(spec, pid, vid, state.concepts, state.counters[:device_exposure]))
        end
        for spec in g.measurements
            push!(state.accum[:measurement], build_measurement(spec, pid, vid, state.concepts, state.counters[:measurement]))
        end
        for spec in g.observations
            push!(state.accum[:observation], build_observation_row(spec, pid, vid, state.concepts, state.counters[:observation]))
        end
        for spec in g.notes
            push!(state.accum[:note], build_note(spec, pid, vid, state.concepts, state.counters[:note]))
        end
    end
